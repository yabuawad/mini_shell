...
static char     *build_replaced(char *old, char *value, char *var, size_t var_index)
{
    ...
    newstr = malloc(ft_strlen(old) - (ft_strlen(var) + 1) + ft_strlen(value) + 1);
    if (!newstr)
        return (NULL);
    while (i < var_index)
    {
...
static void namethisfunction(t_env *myenv, t_cmd *curnt, t_redir *r)
{
    size_t i;
...
t_env   *expand(t_cmd *cmdlist,char **env, int last_exit_status)
{
    ...
    curnt = cmdlist;
    r = NULL;
    namethisfunction(myenv,curnt,r);
    return(myenv);
}   
