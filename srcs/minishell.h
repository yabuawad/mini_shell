...
extern volatile sig_atomic_t	g_global_status;
...
void							execve_handler(char *path, t_cmd *cmd,
									t_env *shell);
...
int								dollar_sign_handler(t_env *myenv, char **str,
									int i);
char							**tokenise(char *line);
void							replace_var(t_env *myenv, char *var, char **str,
									size_t var_index);
char							*removeqt(char *str, int j, size_t i);
int								handled_errors(char **tokens, int i, int dqt,
									int sqt);
t_cmd							*parse(char **tokens);
t_cmd							*addnode(void);
t_redir							*addredir(void);
void							print_argv(char **argv);
int								heredoc_input(t_redir *redir, int fd[2],
									t_env *env);
int								fill_argv(t_cmd *cmd, char **tempargv,
									int tmpsize);
t_cmd							*end_cmd(t_parse **parse, char **argv_temp);
void							set_redtype(t_redir *cmdredir, char **tokens,
									int i);
int								fill_red(t_parse **parse, char **tokens);
void							lastcmd(t_cmd *current, char **argv_temp,
									t_redir *redir_head, int tmpsize);
...
char							**split_argv(char **argv, int pos,
									char **expanded);
int								is_pure_var(char *str);
void							check_tok(t_cmd *curnt, t_env *myenv,
									char ***split, int *i);
